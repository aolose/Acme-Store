...
export const Cube = ({
  ...
  return (
    <div className={clsx(c.baseCubeXYZ, className)} style={style}>
      <div className={clsx(c.baseCubeRotate, isPyramid && c.basePyramid)}>
        {show(Surface.BOTTOM) && (
          <div className={clsx(c.baseWallBottom, bottom)}>{bottomElement}</div>
        )}
        {show(Surface.BACK) && (
          <div className={c.baseWallBack}>
            <div className={clsx(c.innerBack, back)}>{backElement}</div>
          </div>
        )}
        {show(Surface.RIGHT) && (
          <div className={c.baseWallRight}>
            <div className={clsx(c.innerRight, right)}>{rightElement}</div>
          </div>
        )}
        {show(Surface.LEFT) && (
          <div className={c.baseWallLeft}>
            <div className={clsx(c.innerLeft, left)}>{leftElement}</div>
          </div>
        )}
        {show(Surface.TOP) && (
          <div className={clsx(c.baseWallTop, top)}>{topElement}</div>
        )}
        {show(Surface.FRONT) && (
          <div className={c.baseWallFront}>
            <div className={clsx(c.innerFront, front)}>{frontElement}</div>
          </div>
        )}
