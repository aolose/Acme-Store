...
export const Cart = () => {
  ...
  return (
    <>
      <div
        data-test={"cart"}
        className={clsx("fixed flex flex-col rounded-lg", t.cart, act && "act")}
      >
        <div className={"color-light p-2 px-8 mt-2"}>
          SHOPPING CART
          <button
            onClick={() => show(false)}
            className={"clo i-carbon:close-large"}
          ></button>
        </div>
        <div className={"flex-1 overflow-auto rounded-lg"}>
          {items.map(({ quantity, item }) => (
            <div
              ...
              data-test={"cart-item"}
              className={
                "cart-item flex m-3 p-2 flex-wrap " +
                "rounded-sm overflow-hidden"
              }
            >
              <div
                className={
                  "w-16 flex-shrink-0 h-16 rounded-md mr-3 " +
              ...
              <div className={"tools w-full flex text-sm flex items-center"}>
                <div className={"opt flex flex-1"}>
                  ...
                  <span>{quantity}</span>
                  <button data-test={"add"} onClick={() => addToCard(item)}>
                    +
                  </button>
                </div>
                <button
                  ...
                  className={"i-carbon:trash-can text-lg"}
                ></button>
              </div>
            </div>
          ))}
        ...
        <div className={"total " + orbitron.className}>
          <span>Total</span>
          <span className={"t-cur"}>{fmt(total, currency)}</span>
        </div>
        <button onClick={checkout} data-test={"checkout"} className={"check"}>
