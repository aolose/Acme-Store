...
const PlusOne = ({ id = 1 }) => {
  ...
  return (
    <span className={clsx(s && "act", t.plus)} style={style}>
      +1
    </span>
  );
};
...
export const Product = ({
  ...
  return (
    ...
    >
      ...
      <p data-test={"title"} className={"color-light text-sm-0"}>
        {data.title}
      </p>
      ...
      <p className={"color-amber text-sm-1"}>
        <span data-test={"price"}>{fmt(data.price, currency)}</span>
      </p>
    </div>
  );
};
